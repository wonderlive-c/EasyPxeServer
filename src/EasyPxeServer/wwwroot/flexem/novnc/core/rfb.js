...
export default class RFB extends EventTargetMixin {
    constructor(target, url, options) {
        ...
        this._mouse_buttonMaskLastMs = 0;
        this._mouse_arr = [];
    ...
    _screenSize() {
        ...
        // changed for desktop client by wwb
        if (window.navigator.userAgent.indexOf('FBoxChef/1.0')!==-1) {
            size.w = document.documentElement.clientWidth;
            size.h = document.documentElement.clientHeight;
        }
        else if (/Android|webOS|iPhone|iPod|Mobile/i.test(window.navigator.userAgent)) {
            size.w = document.documentElement.clientWidth;
            size.h = document.documentElement.clientHeight;
        }
        else if ((size.w===0 && size.h!==0) || (size.w!==0 && size.h===0)) {
            size.w = document.documentElement.clientWidth;
            size.h = document.documentElement.clientHeight;
        }
        return size;
    ...
    _handleMouseButton(x, y, down, bmask) {
        if (down) {
            ...
            this._mouse_buttonMaskLastMs = Date.now();
        } else {
            this._mouse_buttonMask &= ~bmask;
        }
    ...
    _handleMouseMove(x, y) {
        if (this._mouse_buttonMask == 0) { return; } // Skip when mouse not click down
        ...
        const delay = Date.now() - this._mouse_buttonMaskLastMs;
        if (delay < this.dragDelayMs) {
            return;
        }
        this._mouse_buttonMaskLastMs += delay;
        RFB.messages.pointerEvent(this._sock, this._display.absX(x), this._display.absY(y), this._mouse_buttonMask);
    }
